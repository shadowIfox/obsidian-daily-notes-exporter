...
check('раскладка формы 1440: категория/приоритет во втором ряду, заметка/подпункты/кнопка — каждое на всю ширину своим рядом', (await ev(`(() => { const r = s => document.querySelector(s).getBoundingClientRect(); const c = r('.task-form__category'), p = r('.task-form__priority'), n = r('.task-form__notes'), sub = r('.task-form__subtasks'), b = r('.task-form__submit'), t = r('.task-form__text'), d = r('.task-form__date'), tm = r('.task-form__time'); return Math.abs(c.top - p.top) < 30 && Math.abs(t.top - d.top) < 30 && Math.abs(d.top - tm.top) < 30 && n.top > c.bottom && Math.abs(n.left - c.left) < 2 && sub.top > n.bottom && Math.abs(sub.left - c.left) < 2 && b.top > sub.bottom && Math.abs(b.width - n.width) < 2 && document.documentElement.scrollWidth <= window.innerWidth; })()`)) === true);
