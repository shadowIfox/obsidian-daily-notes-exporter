...
describe('экспорт: Markdown / CSV / PDF-HTML', () => {
    describe('на данных, которые раньше давали «undefined»', () => {
        ...
        it('CSV: BOM и экранирование кавычек и запятых', async () => {
            await fill();
        ...
        it('PDF (HTML): заметка экранируется', async () => {
            await fill();
    ...
    it('только активные привычки и дни по графику', async () => {
        await seed.habits([
    ...
    describe('задачи со временем, приоритетом и заметками', () => {
        ...
        it('HTML и скрипты в названии и заметке не попадают в документ как разметка', async () => {
            await seed.tasks([task({ id: 'x', text: '<b>x</b>', date: T, notes: '<script>alert(1)</script>' })]);
            await seed.habits([]);
            await seed.mood([]);
            const html = buildExport({ period: 'day', category: 'tasks', format: 'pdf' }, T).content;
